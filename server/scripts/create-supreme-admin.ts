...
// Function to add existing user to all organizations
async function addExistingUserToAllOrgs(userEmail: string) {
	...
	try {
		...
		console.log(`\n🎉 User added to all organizations!`);
		console.log(`Added to ${addedCount} new organizations`);
	} catch (error) {
...
async function main() {
	...
	if (command === "add-existing") {
		const userEmail = await askQuestion(
			"Enter the email of the existing user to add to all organizations",
			process.argv[3],
		);
		if (!userEmail) {
		...
		await addExistingUserToAllOrgs(userEmail);
	} else {
...
export { createSupremeAdmin, addExistingUserToAllOrgs };