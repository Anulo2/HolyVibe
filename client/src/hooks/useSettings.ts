...
export const useOrganizationInfoQuery = (
	...
	return useQuery({
		queryKey: ["organization-info", organizationId, refreshToken],
		queryFn: async () => {
...
export const useOrganizationStatsQuery = (
	...
	return useQuery({
		queryKey: ["organization-stats", organizationId, refreshToken],
		queryFn: async () => {
...
export const useOrganizationEventsQuery = (
	...
	return useQuery({
		queryKey: ["organization-events", organizationId, options, refreshToken],
		queryFn: async () => {
...
export const useActiveOrganization = () => {
	...
	// Validate that the selected organization still exists
	useEffect(() => {
		if (
			activeOrganizationId &&
			organizationsQuery.data &&
			...
			const orgExists = organizationsQuery.data.some(
	...
	const setActiveOrganizationId = (id: string | null) => {
		...
		// Force complete refresh of organization-dependent data when it changes
		if (previousId !== id && id) {
			// Increment refresh token to force fresh queries
			setRefreshToken((prev) => prev + 1);

			// Remove all queries from cache to force fresh data
