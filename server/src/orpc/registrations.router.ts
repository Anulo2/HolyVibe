...
export const registrationsRouter = os.router({
  ...
  // Update registration by parent (limited fields)
  updateByParent: withAuth
    .input(
      z.object({
        id: z.string(),
        notes: z.string().optional(),
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        // Get the registration with child and event info
        ...
        const { registration, child, event } = registrationData[0];
        ...
        await checkFamilyMembership(child.familyId, userId);

        // Check if registration can be edited (must be pending and event must be open)
        if (registration.status !== "pending") {
          throw new ORPCError("FORBIDDEN", {
            message: "Can only edit pending registrations",
          });
        }

        if (event.status !== "open") {
          throw new ORPCError("FORBIDDEN", {
            message: "Cannot edit registration for closed events",
          });
        }
        ...
        if (new Date(event.startDate) < new Date()) {
          throw new ORPCError("FORBIDDEN", {
            message: "Cannot edit registration for past events",
          });
        }
        ...
        // Handle authorized persons updates
        if (input.authorizedPersonIds !== undefined) {
          // Validate that all authorized persons belong to the same family
          if (input.authorizedPersonIds.length > 0) {
            const authorizedPersonsData = await db
              .select()
              ...
              .where(
                and(
                  ...
                  eq(authorizedPersons.familyId, child.familyId),
                ),
              );

            if (
              authorizedPersonsData.length !== input.authorizedPersonIds.length
            ) {
              throw new ORPCError("FORBIDDEN", {
                message: "Some authorized persons do not belong to your family",
              });
            }
          }
        ...
        // Handle location authorizations updates
        if (input.locationAuthorizations !== undefined) {
          // Validate that all authorized persons in location authorizations belong to the family
          const personIds = input.locationAuthorizations.map(
            (auth) => auth.authorizedPersonId,
          );
          if (personIds.length > 0) {
            const authorizedPersonsData = await db
              ...
              .where(
                and(
                  inArray(authorizedPersons.id, personIds),
                  eq(authorizedPersons.familyId, child.familyId),
                ),
              );

            if (authorizedPersonsData.length !== new Set(personIds).size) {
              throw new ORPCError("FORBIDDEN", {
                message: "Some authorized persons do not belong to your family",
              });
            }
