...
class FileUploadService {
  ...
  private async uploadToLocal(
    ...
    console.log("📁 Local upload config check:", this.config.local);
    ...
    try {
      ...
      console.log("📁 Directory created/verified:", path.dirname(fullPath));
...
export function createFileUploadService(): FileUploadService {
  const provider =
    (process.env.FILE_UPLOAD_PROVIDER as "cloudinary" | "aws-s3" | "local") ||
    "local";
  ...
  console.log("🔧 Final file upload config:", {
