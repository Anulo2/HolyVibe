...
export default function PhoneTestPage() {
  ...
  const formatForDisplay = (phoneNumber: string): string => {
    try {
      ...
      return parsed && parsed.isValid() ? parsed.formatInternational() : phoneNumber;
    } catch {
      return phoneNumber;
    }
  };
    ...
    <div className="container mx-auto p-6 max-w-4xl">
      ...
      <div className="space-y-8">
        ...
        <div className="bg-card border rounded-lg p-6">
          <h2 className="text-xl font-semibold mb-4">Interactive Phone Input</h2>
          <div className="space-y-4">
            ...
            <div className="bg-muted p-3 rounded">
              ...
              {phoneNumber && (
                <p className="text-sm">
                  <strong>Formato display:</strong> {formatForDisplay(phoneNumber)}
                </p>
              )}
            </div>
          </div>
              ...
              <table className="w-full border-collapse border border-border">
                <thead>
                  <tr className="bg-muted">
                    <th className="border border-border p-2 text-left">Input</th>
                    <th className="border border-border p-2 text-left">Normalizzato (E.164)</th>
                    <th className="border border-border p-2 text-left">Formato Display</th>
                    <th className="border border-border p-2 text-center">Valido</th>
                  </tr>
                </thead>
                <tbody>
                  {testResults.map((result, index) => (
                    <tr key={index} className={result.isValid ? "" : "bg-destructive/10"}>
        ...
        <div className="bg-card border rounded-lg p-6">
          ...
          <ul className="space-y-2 text-sm">
            <li className="flex items-start gap-2">
              ...
              <span><strong>+39 3248921948</strong> → Formato completo internazionale</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-2 h-2 bg-green-500 rounded-full mt-2" />
              <span><strong>393248921948</strong> → Aggiunge + automaticamente</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-2 h-2 bg-green-500 rounded-full mt-2" />
              <span><strong>3248921948</strong> → Aggiunge prefisso +39</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-2 h-2 bg-green-500 rounded-full mt-2" />
              <span><strong>324 892 19 48</strong> → Rimuove spazi e normalizza</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-2 h-2 bg-green-500 rounded-full mt-2" />
              <span><strong>0324 892 1948</strong> → Rimuove zero iniziale</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-2 h-2 bg-red-500 rounded-full mt-2" />
              <span><strong>invalid</strong> → Formati non validi vengono rifiutati</span>
            </li>
          </ul>
        </div>
        ...
        <div className="bg-card border rounded-lg p-6">
          ...
          <ol className="list-decimal list-inside space-y-2 text-sm">
            ...
            <li>I numeri italiani senza prefisso ricevono automaticamente +39</li>
          </ol>
        </div>
      </div>
    </div>
