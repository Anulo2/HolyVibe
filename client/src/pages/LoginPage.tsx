...
export default function LoginPage({ redirectUrl }: LoginPageProps) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data) {
        // User is authenticated - auto-assign to organization if needed
        try {
          const assignmentResult =
            await orpcClient.user.autoAssignToOrganization();
          if (assignmentResult.success && assignmentResult.data.assigned) {
            console.log(
              "User auto-assigned to organization:",
              assignmentResult.data.organizationName,
            );
          }
        } catch (assignmentError) {
          console.error(
            "Error auto-assigning user to organization:",
            assignmentError,
          );
          // Don't fail the login process if assignment fails
        }

        // Profile completion will be handled by ProfileCompletionGuard
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSendOtp}>
            ...
            <div>
              <button
                type="submit"
                disabled={isLoading}
                className="group relative flex w-full justify-center rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:opacity-50"
              >
          ...
          <form className="mt-8 space-y-6" onSubmit={handleVerifyOtp}>
            ...
            <div className="flex space-x-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="flex w-full justify-center rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring disabled:opacity-50"
              >
