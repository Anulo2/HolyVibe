...
import { Building2, Calendar, Info, Settings, User } from "lucide-react";
...
function ParrocchiePage() {
	...
	const { activeOrganizationId, refreshToken } = useActiveOrganization();
				...
				>
					...
					<TabsContent value="informazioni" className="space-y-6">
						{currentOrganization ? (
							<>
								<OrganizationStats
									key={`stats-${activeOrganizationId}-${refreshToken}`}
								...
								<div className="grid gap-6 lg:grid-cols-2">
									<OrganizationInfoCard
										key={`info-${activeOrganizationId}`}
										organization={currentOrganization}
										isLoading={organizationInfoQuery.isLoading}
										showRole={false}
									/>
									<OrganizationQuickEvents
										key={`events-${activeOrganizationId}-${refreshToken}`}
										organizationId={activeOrganizationId || undefined}
										limit={3}
										refreshToken={refreshToken}
									/>
					...
					<TabsContent value="eventi" className="space-y-6">
						{currentOrganization ? (
							<div className="space-y-6">
								<div className="grid gap-6 lg:grid-cols-2">
									<OrganizationQuickEvents
										key={`events-tab-${activeOrganizationId}-${refreshToken}`}
										organizationId={activeOrganizationId || undefined}
										limit={10}
										refreshToken={refreshToken}
									/>
